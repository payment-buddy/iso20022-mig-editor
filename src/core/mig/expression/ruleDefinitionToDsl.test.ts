...
const complex = (mustBe: string, onCondition: string) =>
  rd(
    `<ComplexRule xsi:type="ComplexRule">${mustBe}${onCondition}</ComplexRule>`
  )
...
describe("ruleDefinitionToDsl — supported shapes", () => {
  ...
  it("ComplexRule: presence required when a value matches (real OriginalActivation rule)", () => {
    ...
    expect(dsl(xml)).toBe(
      "not(Status/Code = 'Accepted') or OriginalActivationReference"
    )
  })
  ...
  it("drops a trailing leaf [1] in a presence/absence path (first occurrence ⟺ present)", () => {
    ...
    expect(dsl(xml)).toBe(
      "OriginalPaymentInformationAndStatus and not(Removal)"
    )
  })
  ...
  it("does NOT drop [1] mid-path or before an attribute", () => {
    ...
    expect(
      skip(
        simple(
          `<mustBe><connector>AND</connector>${presence("/A[1]/B")}</mustBe>`
        )
      )
    ).toMatch(/unsupported/)
    ...
    expect(
      skip(
        simple(
          `<mustBe><connector>AND</connector>${presence("/Amt[1]/@Ccy")}</mustBe>`
        )
      )
    ).toMatch(/unsupported/)
  })
  ...
  it("every supported output re-parses cleanly in the DSL", () => {
    const samples = [
      simple(
        `<mustBe><connector>OR</connector>${presence("/A")}${presence("/B")}</mustBe>`
      ),
      complex(
...
describe("ruleDefinitionToDsl — code-set membership with a resolver", () => {
  ...
  it("expands WithInList to an OR-chain over wire codes", () => {
    const xml = simple(
      `<mustBe><connector>AND</connector>${withInList("/Tp", "Cheque3Code")}</mustBe>`
    )
  ...
  it("still fails closed without a resolver, or when the set is unresolved", () => {
    ...
    const unknown = simple(
      `<mustBe><connector>AND</connector>${withInList("/Tp", "MysterySet")}</mustBe>`
    )
...
describe("ruleDefinitionToDsl — occurrence-count idiom (leaf [n>=2])", () => {
  ...
  it("maps Presence(/X[2]) to count(X) >= 2 and Absence(/X[2]) to count(X) < 2", () => {
    expect(
      dsl(
        simple(
          `<mustBe><connector>AND</connector>${card("Presence", "/Tx[2]")}</mustBe>`
        )
      )
    ).toBe("count(Tx) >= 2")
    expect(
      dsl(
        simple(
          `<mustBe><connector>AND</connector>${card("Absence", "/Tx[2]")}</mustBe>`
        )
      )
    ).toBe("count(Tx) < 2")
  })
  ...
  it("does not apply the idiom to a multi-step indexed path (count would be ambiguous)", () => {
    expect(
      skip(
        simple(
          `<mustBe><connector>AND</connector>${card("Presence", "/A/B[2]")}</mustBe>`
        )
      )
    ).toMatch(/unsupported/)
  })
...
describe("ruleDefinitionToDsl — uniformity (all-equal)", () => {
  ...
  it("recognises the pattern in either operand order", () => {
    expect(
      dsl(
        simple(
          `<mustBe><connector>AND</connector>${eqNode("/X[1]/V", "/X[*]/V")}</mustBe>`
        )
      )
    ).toBe("all-equal(X/V)")
  })
  ...
  it("still skips a non-uniformity [*] comparison (every-vs-other-field)", () => {
    expect(
      skip(
        simple(
          `<mustBe><connector>AND</connector>${eqNode("/Item[*]/Ccy", "/Total/Ccy")}</mustBe>`
        )
      )
    ).toMatch(/unsupported/)
  })
