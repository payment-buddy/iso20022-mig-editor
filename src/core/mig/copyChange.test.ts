...
function mig(overrides: ElementOverrides, name = "M"): MessageImplementationGuide {
...
describe("applyFieldCopy — scalar fields", () => {
  it("copies a declared value into the target", () => {
    ...
    const next = applyFieldCopy(src, tgt, P, { type: "field", field: "maxLength" })
  ...
  it("copies a null (cleared) value verbatim rather than collapsing to inherit", () => {
    ...
    const next = applyFieldCopy(src, tgt, P, { type: "field", field: "pattern" })
  ...
  it("copies array fields (allowedValues)", () => {
    ...
    const tgt = mig({ [P]: {} })
    const next = applyFieldCopy(src, tgt, P, { type: "field", field: "allowedValues" })
...
describe("applyFieldCopy — annotations", () => {
  it("sets one annotation, leaving the target's others intact", () => {
    ...
    const tgt = mig({ [P]: { annotations: { Usage: "Optional", Note: "keep" } } })
    const next = applyFieldCopy(src, tgt, P, { type: "annotation", name: "Usage" })
    expect(next.elementOverrides[P].annotations).toEqual({ Usage: "Required", Note: "keep" })
  })
  ...
  it("clears the annotations field when the last entry is removed", () => {
    const src = mig({ [P]: {} })
    const tgt = mig({ [P]: { maxLength: 18, annotations: { Usage: "Optional" } } })
    const next = applyFieldCopy(src, tgt, P, { type: "annotation", name: "Usage" })
...
describe("applyFieldCopy — additional constraints", () => {
  it("adds a constraint the target lacks", () => {
    const src = mig({ [P]: { additionalConstraints: [{ name: "R1", definition: "positive" }] } })
    const tgt = mig({ [P]: {} })
  ...
  it("overwrites an existing constraint to mirror the source", () => {
    const src = mig({
      [P]: { additionalConstraints: [{ name: "R1", definition: "non-negative", expression: "a>=0" }] },
    })
    const tgt = mig({
      [P]: { additionalConstraints: [{ name: "R1", definition: "positive", annotations: { S: "1" } }] },
    })
    const next = applyFieldCopy(src, tgt, P, { type: "constraint", name: "R1" })
    const c = next.elementOverrides[P].additionalConstraints![0]
    expect(c).toEqual({ name: "R1", definition: "non-negative", expression: "a>=0" })
  })

  it("removes a constraint absent in the source", () => {
    const src = mig({ [P]: {} })
    const tgt = mig({ [P]: { additionalConstraints: [{ name: "R1", definition: "x" }] } })
    const next = applyFieldCopy(src, tgt, P, { type: "constraint", name: "R1" })
    expect(P in next.elementOverrides).toBe(false)
  })
})
